...
func (e *engine) paramSetupFn(params []api.ValueType, results []api.ValueType) (ret []byte, err error) {
	if runtime.GOARCH == "arm64" {
		return e.paramSetupFnArm64(params, results)
	} else if runtime.GOARCH == "amd64" {
		return e.paramSetupFnAmd64(params, results)
	} else {
		panic(fmt.Sprintf("BUG: unsupported GOARCH: %s", runtime.GOARCH))
	}
}

func funcTypeKey(params []api.ValueType, results []api.ValueType) string {
	builder := strings.Builder{}
	builder.Grow(len(params) + len(results) + 1)
	builder.Write(params)
	builder.WriteByte(0)
	builder.Write(results)
	return builder.String()
}

// paramSetupFnArm64 implements paramSetupFn for arm64 architecture.
func (e *engine) paramSetupFnArm64(params []api.ValueType, results []api.ValueType) (ret []byte, err error) {
	const (
...
func (e *engine) paramSetupFnAmd64(params []api.ValueType, results []api.ValueType) (ret []byte, err error) {
	panic("TODO")
}
